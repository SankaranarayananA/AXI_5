...
This repository is a local AXI5-focused fork of the upstream `pulp-platform/axi` codebase.

- Upstream copyright, authorship, and licensing are preserved.
- New fork additions are focused on AXI5 extension components, including NSAID-aware interfaces and adapters.
- Newly added AXI5 files in `src/` are marked with explicit fork-attribution headers.
